            ...
            public void onClick(View v) {
                ...
                if(save==1) {
                    try {
                        ...
                        //更新聊天内容
                        updateChatView(new ChatEntity(Integer.parseInt(myAvatar), chatContent, nowtime, 2));
                    }
                    catch (Exception e){
    ...
    public class MyBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if(mes[Message.TYPE].equals(ChatMessageType.COM_MES)) {
                //更新聊天内容
                updateChatView(new ChatEntity(Integer.parseInt(mes[Message.SENDERAVATAR]),mes[Message.CONTENT], mes[Message.TIME], 0));
                //标记此消息为已读
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            if (message.getString("type").equals("0")) {
                                if (message.getString("userid").equals(myAccount)) {
                                    chatEntityList.add(new ChatEntity(Integer.parseInt(myAvatar), message.getString("content"), message.getString("createtime"), 2));
                                } else {
                                    if (message.getString("userPic").equals("null")) {
                                        chatEntityList.add(new ChatEntity(1, message.getString("content"), message.getString("createtime"), 0));
                                    } else {
                                        chatEntityList.add(new ChatEntity(Integer.parseInt(message.getString("userPic")), message.getString("content"), message.getString("createtime"), 0));
                                    }
                                }
                            } else {
